...
struct FpsStats {
    ...
    // 构造函数
    FpsStats(const std::string& module, const std::string& secondary = "", int interval = 1000)
...
private:
    void print_stats() const {
        ...
        int fps = static_cast<int>(fps_count * scale);
        int secondary = static_cast<int>(secondary_count * scale);
        ...
        if (secondary_label.empty()) {
            if (total_latency > 0) {
                debug::print(debug::PrintMode::DEBUG, module_name,
                    "FPS: {}, avg_latency: {:.1f}ms{}", fps, avg_latency, extra);
            } else {
                debug::print(debug::PrintMode::DEBUG, module_name,
                    "FPS: {}{}", fps, extra);
            }
            ...
            if (total_latency > 0) {
                debug::print(debug::PrintMode::DEBUG, module_name,
                    ...
                    fps, secondary_label, secondary, avg_latency, extra);
            } else {
                debug::print(debug::PrintMode::DEBUG, module_name,
                    "FPS: {}, {}: {}{}", fps, secondary_label, secondary, extra);
            }
        }
    }
