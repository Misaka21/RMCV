...
inline std::unique_ptr<Detector> create_detector_from_config(EnemyColor color) {
    ...
    std::string model_type_str = static_param::get_param<std::string>(
        config, "Detector.traditional.classify", "model_type");
    ...
    detector->classifier = std::make_unique<NumberClassifier>(
        full_model_path, full_label_path, classify_threshold, model_type, ignore_classes
    );
...
inline std::unique_ptr<Detector> create_detector(
    ...
    detector->classifier = std::make_unique<NumberClassifier>(
        model_path, label_path, classify_threshold, model_type, ignore_classes
    );
