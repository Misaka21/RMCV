 ...
 *      solver = "rk4_quadratic"  # 或 "analytic_linear", "rk4_linear"
 *
 *   2. 代码中使用:
 *      auto result = trajectory::solve(target, bullet_speed);
 *      // 自动根据配置选择求解器
 *
 *   3. 强制使用特定求解器:
 *      auto result = trajectory::solve(target, bullet_speed, SolverType::RK4_QUADRATIC);
 */
    ...
    SolverType solver_type)
{
    switch (solver_type) {
        case SolverType::ANALYTIC_LINEAR: {
            static TrajectorySolver analytic_solver;
