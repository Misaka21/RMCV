...
│   └── outpost_motion.*   # 前哨站模型 (7维 EKF, namespace: outpost_model)
│
└── enemy_state/
...
void OutpostMotion::constrain_omega() {
    ...
    if (!omega_sign_determined_) {
        ...
        if (omega > OMEGA_THRESHOLD) {
            x[outpost_model::OMEGA] = +0.8π;  // 逆时针
            omega_sign_determined_ = true;
        } else if (omega < -OMEGA_THRESHOLD) {
            x[outpost_model::OMEGA] = -0.8π;  // 顺时针
            omega_sign_determined_ = true;
        }
    } else {
        // 已确定，只约束绝对值
        x[outpost_model::OMEGA] = copysign(0.8π, omega);
    }
}
```
